...
class MockHandler implements \Countable
{
    ...
    public function __invoke(\YaySMTP\Aws3\Aws\CommandInterface $command, \YaySMTP\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        if (!$this->queue) {
        ...
        if ($result instanceof \Exception) {
            $result = new \YaySMTP\Aws3\GuzzleHttp\Promise\RejectedPromise($result);
        } else {
            ...
            $result = \YaySMTP\Aws3\GuzzleHttp\Promise\promise_for($result);
        }
