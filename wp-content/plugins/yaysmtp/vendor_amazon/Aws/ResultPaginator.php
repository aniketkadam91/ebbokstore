...
class ResultPaginator implements \Iterator {
   ...
   * @param array              $config
   */
  public function __construct(\YaySMTP\Aws3\Aws\AwsClientInterface $client, $operation, array $args, array $config) {
    $this->client = $client;
  ...
  public function each(callable $handleResult) {
    return \YaySMTP\Aws3\GuzzleHttp\Promise\coroutine(function () use ($handleResult) {
      $nextToken = null;
      do {
        ...
        if ($retVal !== null) {
          (yield \YaySMTP\Aws3\GuzzleHttp\Promise\promise_for($retVal));
        }
      } while ($nextToken);
    });
  }
  ...
  public function search($expression) {
    ...
    return flatmap($this, function (\YaySMTP\Aws3\Aws\Result $result) use ($expression) {
