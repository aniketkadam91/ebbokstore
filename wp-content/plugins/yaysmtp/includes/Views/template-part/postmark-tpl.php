...
if ( ! empty( $params ) ) {
	if ( ! empty( $settings[ $mailer ] ) && isset( $settings[ $mailer ]['api_key'] ) ) {
		$postmarkApiKey = $settings[ $mailer ]['api_key'];
	}
}
?>
  ...
  <div class="yay-smtp-card-header">
	<div class="yay-smtp-card-title-wrapper">
	  <h3 class="yay-smtp-card-title yay-smtp-card-header-item">
		Step 3: Config for Postmark
		<div class="yay-tooltip doc-setting">
		  <a class="yay-smtp-button" href="https://yaycommerce.gitbook.io/yaysmtp/how-to-set-up-smtps/how-to-connect-postmark/" target="_blank">
		  ...
		  <span class="yay-tooltiptext yay-tooltip-bottom">Postmark Documentation</span>
		</div>
	  </h3>
	  <h3 class="yay-smtp-card-description yay-smtp-card-header-item">
		Postmark delivers your email to customers on time, every time
	  </h3>
	</div>
  </div>
  <div class="yay-smtp-card-body">
	<div class="setting-el">
	  ...
	  <div class="setting-field">
		<input data-setting="api_key" type="password" spellcheck="false" class="yay-settings" value="<?php echo esc_attr( $postmarkApiKey ); ?>">
		<p class="setting-description">
		  To get API Key. Click here to
		  <a href="https://account.postmarkapp.com/servers" target="_blank" rel="noopener noreferrer">Postmark Servers </a>
		  and choose API Tokens tab.
		</p>
	  </div>
	</div>
  </div>
