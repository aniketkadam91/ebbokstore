...
if ( ! empty( $params ) && ! empty( $settings[ $mailer ] ) ) {
	...
	if ( isset( $settings[ $mailer ]['domain'] ) ) {
		$domainV = $settings[ $mailer ]['domain'];
	}
	if ( isset( $settings[ $mailer ]['region'] ) ) {
		$region = $settings[ $mailer ]['region'];
	}
}
$mailgunNote = __( 'Choose which region (North America or Europe) you want your message data to be processed by. More details at https://mailgun.com/regions/', 'yay-smtp' );
?>
  ...
  <div class="yay-smtp-card-body">
	<div class="setting-el">
	  ...
	  <div class="setting-field">
		<input data-setting="api_key" type="password" spellcheck="false" class="yay-settings" value="<?php echo esc_attr( $mailgunApiKey ); ?>">
	...
	<div class="setting-el">
	  ...
	  <div class="setting-field">
		<input type="text" data-setting="domain" class="yay-settings" value="<?php echo esc_attr( $domainV ); ?>">
		<p class="setting-description">
		  Click here to
	  ...
	  <div class="setting-label">
		...
		<div class="yay-tooltip icon-tootip-wrap">
		  ...
		  <span class="yay-tooltiptext yay-tooltip-bottom" style="font-size:12px;font-weight: normal;max-width: 250px;">
			<?php echo wp_kses_post( $mailgunNote ); ?>
		  </span>
		</div>
	  </div>
	  <div class="setting-field">
		<label class="radio-setting">
		  <input type="radio" data-setting="region" name="<?php echo empty($isFallBack) ? 'yay_smtp_setting_mailgun_region' : 'yay_smtp_setting_mailgun_region_fallback' ?>" class="yay-settings" value="US" <?php echo 'US' === $region ? 'checked' : ''; ?>>
		  US
		</label>
		<label class="radio-setting">
		  <input type="radio" data-setting="region" name="<?php echo empty($isFallBack) ? 'yay_smtp_setting_mailgun_region' : 'yay_smtp_setting_mailgun_region_fallback' ?>" class="yay-settings" value="EU" <?php echo 'EU' === $region ? 'checked' : ''; ?>>
		  EU
		</label>
	  </div>
	</div>
