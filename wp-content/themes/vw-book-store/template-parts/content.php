  ...
  <div class="post-main-box wow zoomInUp delay-1000" data-wow-duration="2s">
    ...
    if($vw_book_store_theme_lay == 'Default'){ ?>
      <div class="row">
        ...
        <?php } ?>
        <div class="new-text <?php if(has_post_thumbnail() && get_theme_mod( 'vw_book_store_featured_image_hide_show',true) == 1) { ?>col-lg-6 col-md-6"<?php } else { ?>col-lg-12 col-md-12 <?php } ?>">
          <h2 class="section-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo the_title_attribute(); ?>"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
          <?php if( get_theme_mod( 'vw_book_store_toggle_postdate',true) == 1 || get_theme_mod( 'vw_book_store_toggle_author',true) == 1 || get_theme_mod( 'vw_book_store_toggle_comments',true) == 1 || get_theme_mod( 'vw_book_store_toggle_time',true) == 1) { ?>
            <div class="post-info">
              <?php if(get_theme_mod('vw_book_store_toggle_postdate',true)==1){ ?>
                <i class="<?php echo esc_attr(get_theme_mod('vw_book_store_toggle_postdate_icon','fas fa-calendar')); ?>"></i><span class="entry-date"><a href="<?php echo esc_url( get_day_link( $vw_book_store_archive_year, $vw_book_store_archive_month, $vw_book_store_archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span>
                <?php echo esc_html(get_theme_mod('vw_book_store_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('vw_book_store_toggle_author',true)==1){ ?>
                <span> <i class="<?php echo esc_attr(get_theme_mod('vw_book_store_toggle_author_icon','fas fa-user')); ?>"></i><span class="entry-author"><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span><?php echo esc_html(get_theme_mod('vw_book_store_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('vw_book_store_toggle_comments',true)==1){ ?>
                 <i class="<?php echo esc_attr(get_theme_mod('vw_book_store_toggle_comments_icon','fa fa-comments')); ?>" aria-hidden="true"></i><span class="entry-comments"><?php comments_number( __('0 Comment', 'vw-book-store'), __('0 Comments', 'vw-book-store'), __('% Comments', 'vw-book-store') ); ?> </span>
                <?php echo esc_html(get_theme_mod('vw_book_store_meta_field_separator', '|'));?></span>
              <?php } ?>

              <?php if(get_theme_mod('vw_book_store_toggle_time',true)==1){ ?>
                 <i class="<?php echo esc_attr(get_theme_mod('vw_book_store_toggle_time_icon','fas fa-clock')); ?>"></i><span class="entry-time"><?php echo esc_html( get_the_time() ); ?></span>
              <?php } ?>
            </div>  
          <?php } ?>    
            ...
            <p>
              ...
              if($vw_book_store_theme_lay == 'Excerpt'){ ?>
                <?php if(get_the_excerpt()) { ?>
                  <?php $vw_book_store_excerpt = get_the_excerpt(); echo esc_html( vw_book_store_string_limit_words( $vw_book_store_excerpt, esc_attr(get_theme_mod('vw_book_store_excerpt_number','30')))); ?> <?php echo esc_html(get_theme_mod('vw_book_store_excerpt_suffix',''));?>
                <?php }?>
              <?php }?>
            </p>
          </div>
    ...
    <?php }else if($vw_book_store_theme_lay == 'Center'){ ?>
      <div class="service-text">
        <h2 class="section-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo the_title_attribute(); ?>"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
        <?php if( get_theme_mod( 'vw_book_store_featured_image_hide_show',true) == 1) { ?>
          <div class="box-image">
            <?php the_post_thumbnail(); ?>
          </div>
        <?php } ?>
